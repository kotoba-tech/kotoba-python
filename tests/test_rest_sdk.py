"""Unit tests for the REST SDK (``transcribe`` = one-shot POST /v1/speech-to-text).
...
from kotoba.errors import APIError, AuthError, ProtocolError, WorkerStartupError

BASE_URL = "http://fake.example"
STT_URL = f"{BASE_URL}/v1/speech-to-text"
...
def _client(url: str = BASE_URL) -> kotoba.KotobaClient:
    return kotoba.KotobaClient(api_key="token-abc", url=url, max_retries=0)
...
@responses.activate
def test_transcribe_posts_one_shot(fake_wav):
    responses.add(
        responses.POST,
        STT_URL,
    ...
    result = _client().asr.transcribe(fake_wav, language="ja")
    assert result.text == "こんにちは"
    assert result.metadata == {"audio_duration_secs": 1.2}
    assert len(responses.calls) == 1  # single request, no polling
    req = responses.calls[0].request
    assert req.headers["Authorization"] == "Bearer token-abc"
    assert req.headers["xi-api-key"] == "token-abc"
    assert b'name="language"' in req.body and b"ja" in req.body
    assert b'name="file"' in req.body and b"tiny.wav" in req.body


@responses.activate
def test_transcribe_tolerates_v1_suffixed_base_url(fake_wav):
    """A base URL written against the old convention (``.../v1``) must not
    produce ``/v1/v1/speech-to-text``."""

    responses.add(responses.POST, STT_URL, json={"text": "ok"}, status=200)
    result = _client(url=f"{BASE_URL}/v1").asr.transcribe(fake_wav)
    assert result.text == "ok"
    assert responses.calls[0].request.url == STT_URL


@responses.activate
def test_transcribe_empty_text(fake_wav):
    responses.add(responses.POST, STT_URL, json={"text": None}, status=200)
    assert _client().asr.transcribe(fake_wav).text == ""


@responses.activate
def test_transcribe_timeout(fake_wav):
    responses.add(
        responses.POST, STT_URL, body=requests.exceptions.ReadTimeout("slow")
    )
    with pytest.raises(kotoba.TimeoutError):
        _client().asr.transcribe(fake_wav, timeout=0.05)
...
def test_protocol_error_on_400(fake_wav):
    responses.add(
        responses.POST, STT_URL, json={"detail": "empty audio payload"}, status=400
    )
...
@responses.activate
def test_api_error_on_5xx(fake_wav):
    responses.add(responses.POST, STT_URL, json={"detail": "boom"}, status=503)
...
def test_protocol_error_preserves_fastapi_validation_detail(fake_wav):
    ...
    responses.add(responses.POST, STT_URL, json={"detail": detail}, status=422)
...
def test_client_requires_url(monkeypatch):
    ...
    with pytest.raises(RuntimeError):
        client.asr.transcribe(BytesIO(b"dummy"))  # type: ignore[arg-type]
...
def test_fal_probe_waits_for_ready_before_post(fake_wav):
    ...
    responses.add(responses.GET, PROBE_URL, status=503)
    responses.add(responses.GET, PROBE_URL, status=200)
    responses.add(responses.POST, STT_URL, json={"text": "cold ok"}, status=200)
    result = _fal_client().asr.transcribe(fake_wav)
    assert result.text == "cold ok"
...
def test_fal_probe_401_raises_auth_error_without_post(fake_wav):
    ...
    with pytest.raises(AuthError):
        _fal_client().asr.transcribe(fake_wav)
...
def test_fal_probe_deadline_raises_worker_startup_error(fake_wav):
    ...
    with pytest.raises(WorkerStartupError):
        _fal_client(provider=tiny).asr.transcribe(fake_wav)


@responses.activate
def test_fal_warmup_probes_once_across_transcribes(fake_wav):
    responses.add(responses.GET, PROBE_URL, status=200)
    responses.add(responses.POST, STT_URL, json={"text": "a"}, status=200)
    responses.add(responses.POST, STT_URL, json={"text": "b"}, status=200)
    client = _fal_client()
    client.warmup()
...
def test_kotoba_provider_never_probes(fake_wav):
    responses.add(responses.POST, STT_URL, json={"text": "k"}, status=200)
    assert _client().asr.transcribe(fake_wav).text == "k"
...
def _async_session(app, provider=_FAST_FAL, api_key="fal-secret") -> AsyncHttpSession:
...
async def test_fal_async_probe_auth_error():
    ...
    session = _async_session(app, api_key="bad")
    with pytest.raises(AuthError):
        await session.ensure_ready()
    await session.aclose()
...
async def test_async_transcribe_one_shot(fake_wav):
    def app(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(200, json={"text": "async ok", "audio_duration_secs": 2})
    ...
    assert result.metadata == {"audio_duration_secs": 2}
    await session.aclose()
